		...
		<?php
			...
			foreach ($buys as $key => $buy) {
				...
				<div class="border-ccc padding-10 order-container">
					<strong>Order Id:</strong> <?php echo $buy['id'] ?>
					<a href="cancel_order.php?id=<?php echo $buy['id']; ?>" class="float-right text-black">Cancel order</a>
					<hr/>
				...
				foreach ($product_id_stack as $key => $product_id) {
					...
					<td class="padding-10 box-sizing"><a href="product.php?id=<?= $product['id'] ?>"><?php echo $product['name']; ?></a></td>
